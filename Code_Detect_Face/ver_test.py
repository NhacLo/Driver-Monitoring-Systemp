...
class DetectionPublisher(Node):
    ...
    def publish_detection(self, warning_label):
        msg = String()
        data = {'label': warning_label}
        msg.data = json.dumps(data) 
        self.publisher_.publish(msg)
        self.get_logger().info(f'Published detection JSON: {msg.data}')
class WarningManager:
    ...
    def evaluate(self, pose, ear):
        ...
        if warning_target == "NORMAL":
            self.current_condition = None
            self.condition_start = None
            return head_label, eye_label, "NORMAL"
        ...
        while True:
            ...
            if pose is not None:
                if not neutral_set:
                    ...
                    reset_angle_state()
                    neutral_set = True
                head_label, eye_label, warning = warning_manager.evaluate(pose, ear)
                ...
                if warning != last_warning:
                    node.publish_detection(warning)
