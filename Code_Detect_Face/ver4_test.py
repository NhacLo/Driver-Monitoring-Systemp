...
class DetectionPublisher(Node):
    def __init__(self):
        ...
        self.publisher_ = self.create_publisher(String, 'head_pose_detection', 10)
        self.get_logger().info("Detection Publisher initialized.")
    def publish_detection(self, warning_label):
        msg = String()
        msg.data = warning_label
...
class WarningManager:
    ...
    def evaluate(self, pose, ear):
        head_label = classify_head_pose(pose["yaw"], pose["pitch"])
        eye_label = classify_eye_state(ear)

        warning_target = None
        delay = 0.0
        if head_label == "LOOK_STRAIGHT" and eye_label == "open":
            warning_target = "NORMAL"
        if head_label == "LOOK_STRAIGHT" and eye_label == "closed":
            warning_target = "WAKEUP"
            delay = 3.0
        elif head_label == "LOOK_DOWN" and eye_label == "closed":
            warning_target = "DANGER"
            delay = 2.0
        elif head_label in ["LOOK_LEFT", "LOOK_RIGHT", "LOOK_UP"]:
            warning_target = "ATTENTION"
            delay = 3.0

        now = time.time()
        #*******************************************************
        if self.alarm_active:
            return head_label, eye_label, None
   
        if warning_target is None:
            self.current_condition = None
            self.condition_start = None
            return head_label, eye_label, None
      
        if self.current_condition != warning_target:
            self.current_condition = warning_target
            self.condition_start = now
            return head_label, eye_label, None

        if now - self.condition_start >= delay:
            self.alarm_active = True
            self.active_warning = warning_target
            return head_label, eye_label, warning_target
...
class DetectionPublisher(Node):
    def __init__(self, warning_manager):
        ...
        self.publisher_ = self.create_publisher(String, 'head_pose_detection', 10)
        self.ack_subscriber_ = self.create_subscription(
            Bool,
            'driver_ack',
            self.ack_callback,
            10
    ...
    def ack_callback(self, msg):
        if msg.data:
            self.warning_manager.clear_alarm()
            self.get_logger().info("Driver acknowledged. Alarm cleared.")
def main():
    global NEUTRAL_R
    ...
    node = DetectionPublisher(warning_manager)
    ...
    try:
        ...
        while True:
            ...
            if pose is not None:
                ...
                head_label, eye_label, warning_to_publish = warning_manager.evaluate(pose, ear)

                cv2.putText(
                    output_frame,
                    f"Direction: {head_label}, Eye: {eye_label}",
                    (20, 90),
                    cv2.FONT_HERSHEY_SIMPLEX,
                    0.7,
                    (255, 255, 0),
                ...
                if warning_manager.alarm_active:
                    cv2.putText(
                        output_frame,
                        f"ALARM: {warning_manager.active_warning}",
                        (20, 130),
                        cv2.FONT_HERSHEY_SIMPLEX,
                        0.8,
                        (0, 0, 255),
                ...
                if warning_to_publish is not None:
                    node.publish_detection(warning_to_publish)
            ...
            if key == ord('c'):
                warning_manager.clear_alarm()
                print("Alarm cleared by driver action.")
            if key == ord('q'):
                break
